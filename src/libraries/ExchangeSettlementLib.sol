...
library ExchangeSettlementLib {
    ...
    function internalSettle(
        ...
        // block.timestamp remove all entries, even if no reclaim and no rebate
    ...
    function _secsLeftInWaitingPeriodForExchange(
        uint timestamp,
        uint waitingPeriod
    ) internal view returns (uint) {
        if (timestamp == 0 || block.timestamp >= timestamp.add(waitingPeriod)) {
            return 0;
        }

        return timestamp.add(waitingPeriod).sub(block.timestamp);
    }
    ...
    function hasWaitingPeriodOrSettlementOwing(
        ...
        (uint reclaimAmount, , , ) = _settlementOwing(
            resolvedAddresses,
            account,
            currencyKey,
            waitingPeriod
        ...
        )
    {
        ...
        numEntries = resolvedAddresses.exchangeState.getLengthOfEntries(
            account,
            currencyKey
        );
        ...
        for (uint i = 0; i < numEntries; i++) {
            ...
            IExchangeState.ExchangeEntry
                memory exchangeEntry = _getExchangeEntry(
                    resolvedAddresses.exchangeState,
                    account,
                    currencyKey,
                    i
            ...
            {
                (uint destinationAmount, , ) = resolvedAddresses
                    .exchangeRates
                ...
                amountShouldHaveReceived = _deductFeesFromAmount(
                    destinationAmount,
            ...
            bool sip65condition = resolvedAddresses
                .circuitBreaker
    ...
    function appendExchange(
        ...
        uint roundIdForSrc = resolvedAddresses.exchangeRates.getCurrentRoundId(
            src
        );
        uint roundIdForDest = resolvedAddresses.exchangeRates.getCurrentRoundId(
            dest
        );
