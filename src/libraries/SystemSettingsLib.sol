...
library SystemSettingsLib {
    ...
    uint public constant MAX_ATOMIC_VOLUME_PER_BLOCK = type(uint192).max;
    ...
    function setLiquidationRatio(
        ...
        uint getSfcxLiquidationPenalty,
        uint getIssuanceRatio
    ) external {
        require(
            _liquidationRatio <=
                MAX_LIQUIDATION_RATIO.divideDecimal(
                    SafeDecimalMath.unit().add(getSfcxLiquidationPenalty)
                ),
    ...
    function updateSfcxLiquidationPenalty(
        IFlexibleStorage flexibleStorage,
        bytes32 settingName,
        uint penalty
    ) external {
    ...
    function setEtherWrapperMintFeeRate(
        ...
        require(
            _rate <= uint(MAX_WRAPPER_MINT_FEE_RATE),
    ...
    function setEtherWrapperBurnFeeRate(
        ...
        require(
            _rate <= uint(MAX_WRAPPER_BURN_FEE_RATE),
