...
contract FuturesMarketManager is Ownable, MixinResolver, IFuturesMarketManager {
    ...
    function totalDebt() external view returns (uint debt, bool isInvalid) {
        ...
        for (uint i = 0; i < numOfMarkets; i++) {
            (uint marketDebt, bool invalid) = IMarketViews(
                _allMarkets.elements[i]
            ).marketDebt();
    ...
    function _marketSummaries(
        address[] memory addresses
    ) internal view returns (MarketSummary[] memory) {
        ...
        for (uint i; i < nMarkets; i++) {
            ...
            summaries[i] = MarketSummary({
                ...
                currentFundingVelocity: proxied
                    ? market.currentFundingVelocity()
                    : int256(0), // v1 does not have velocity.
    ...
    function allEndorsedAddresses() external view returns (address[] memory) {
        return
            _endorsedAddresses.getPage(0, _endorsedAddresses.elements.length);
    }
     ...
     * Add a set of new markets. Reverts if some market key already has a market.
     */
    function addProxiedMarkets(
        address[] calldata marketsToAdd
    ) external onlyOwner {
        uint numOfMarkets = marketsToAdd.length;
        for (uint i; i < numOfMarkets; i++) {
    ...
    function removeMarkets(
        address[] calldata marketsToRemove
    ) external onlyOwner {
     ...
     */
    function removeMarketsByKey(
        bytes32[] calldata marketKeysToRemove
    ) external onlyOwner {
        _removeMarkets(_marketsForKeys(marketKeysToRemove));
    }

    function updateMarketsImplementations(
        address[] calldata marketsToUpdate
    ) external onlyOwner {
     ...
     */
    function removeEndorsedAddresses(
        address[] calldata addresses
    ) external onlyOwner {
    ...
    function _requireIsMarketOrImplementation() internal view {
        require(
            _legacyMarkets.contains(msg.sender) ||
                _implementations.contains(msg.sender),
