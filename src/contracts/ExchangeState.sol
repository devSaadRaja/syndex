...
contract ExchangeState is Ownable, State {
    mapping(address => mapping(bytes32 => IExchangeState.ExchangeEntry[]))
        public exchanges;
    ...
    function appendExchangeEntry(
        ...
        require(
            exchanges[account][dest].length < maxEntriesInQueue,
    ...
    {
        IExchangeState.ExchangeEntry storage entry = exchanges[account][
            currencyKey
        ][index];
        return (
            entry.src,
    ...
    function getMaxTimestamp(
        ...
        IExchangeState.ExchangeEntry[] storage userEntries = exchanges[account][
            currencyKey
        ];
        uint timestamp = 0;
