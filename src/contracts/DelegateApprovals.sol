...
contract DelegateApprovals is Ownable, IDelegateApprovals {
    ...
    function approvedAll(
        ...
        return
            eternalStorage.getBooleanValue(
                _getKey(APPROVE_ALL, authoriser, delegate)
            );
    }
    ...
    function _checkApproval(
        ...
        return
            eternalStorage.getBooleanValue(
                _getKey(action, authoriser, delegate)
            );
    }
    ...
    function _setApproval(
        ...
        eternalStorage.setBooleanValue(
            _getKey(action, authoriser, delegate),
            true
        );
    ...
    function _withdrawApproval(
        ...
        if (
            ...
            eternalStorage.deleteBooleanValue(
                _getKey(action, authoriser, delegate)
            );
    ...
    function setEternalStorage(
        EternalStorage _eternalStorage
    ) external onlyOwner {
        require(
            address(_eternalStorage) != address(0),
