...
contract BaseSynthetix is ExternStateToken, MixinResolver, Blacklist {
    ...
    function getFirstNonZeroEscrowIndex(
        address account
    ) external view returns (uint) {
    ...
    function _canTransfer(
        ...
        address legacyMarketAddress = resolver.getAddress(
            CONTRACT_V3_LEGACYMARKET
        );
        if (
    ...
    function migrateEscrowContractBalance() external onlyOwner {
        ...
        address to = resolver.requireAndGetAddress(
            "RewardEscrowV2",
            "New escrow address unset"
        );
        ...
        returns (bool)
    {
        ...
        (uint totalRedeemed, uint debtRemoved, ) = issuer().liquidateAccount(
            liquidatedAccount,
            true
        );
    ...
    function revokeAllEscrow(address account) external systemActive {
        ...
        require(
            msg.sender == legacyMarketAddress,
            "Only LegacyMarket can revoke escrow"
        );
        rewardEscrowV2().revokeFrom(
            account,
            legacyMarketAddress,
            rewardEscrowV2().totalEscrowedAccountBalance(account),
            0
        );
    }
    ...
    {
        address debtMigratorOnEthereum = resolver.getAddress(
    ...
    function emitAccountLiquidated(
        ...
        proxy._emit(
            abi.encode(snxRedeemed, amountLiquidated, liquidatorAddress),
            2,
