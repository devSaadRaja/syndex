...
import "../libraries/ExchangeSettlementLib.sol";

contract Exchanger is Ownable, MixinSystemSettings, IExchanger {
    ...
    bytes32 private constant CONTRACT_DIRECT_INTEGRATION_MANAGER =
    ...
    function directIntegrationManager()
        internal
        view
        returns (IDirectIntegrationManager)
    {
        return
    ...
    function resolvedAddresses()
        internal
        view
        returns (ExchangeSettlementLib.ResolvedAddresses memory)
    {
        return
    ...
    function lastExchangeRate(
        bytes32 currencyKey
    ) external view returns (uint) {
        return
            circuitBreaker().lastValue(
                address(exchangeRates().aggregators(currencyKey))
            );
    }
    ...
    {
        (reclaimAmount, rebateAmount, numEntries, ) = ExchangeSettlementLib
            .settlementOwing(
                resolvedAddresses(),
                account,
                currencyKey,
    ...
    function calculateAmountAfterSettlement(
        ...
        uint balanceOfSourceAfterSettlement = IERC20(
        ...
        returns (uint amountReceived, IVirtualSynth vSynth)
    {
        ...
        IDirectIntegrationManager.ParameterIntegrationSettings
            memory sourceSettings = _exchangeSettings(from, sourceCurrencyKey);
        IDirectIntegrationManager.ParameterIntegrationSettings
            memory destinationSettings = _exchangeSettings(
                from,
    ...
    function _settleAndCalcSourceAmountRemaining(
        ...
        (, uint refunded, uint numEntriesSettled) = ExchangeSettlementLib
            .internalSettle(
                resolvedAddresses(),
                from,
                sourceCurrencyKey,
    ...
    function _exchange(
        ...
        ExchangeSettlementLib.ResolvedAddresses
            memory addrs = resolvedAddresses();

        entry.roundIdForSrc = addrs.exchangeRates.getCurrentRoundId(
            sourceSettings.currencyKey
        );
        entry.roundIdForDest = addrs.exchangeRates.getCurrentRoundId(
            destinationSettings.currencyKey
        );
        ...
        ) = addrs.exchangeRates.effectiveValueAndRatesAtRound(
            sourceSettings.currencyKey,
        ...
        amountReceived = ExchangeSettlementLib._deductFeesFromAmount(
            entry.destinationAmount,
    ...
    {
        settings = directIntegrationManager().getExchangeParameters(
            from,
            currencyKey
        );
    }
    ...
    function feeRateForExchange(
        ...
        IDirectIntegrationManager.ParameterIntegrationSettings
            memory sourceSettings = _exchangeSettings(
                msg.sender,
                sourceCurrencyKey
            );
        IDirectIntegrationManager.ParameterIntegrationSettings
        ...
        (uint feeRate, bool tooVolatile) = _feeRateForExchange(
            sourceSettings,
            destinationSettings
        );
    ...
    function _dynamicFeeRateForExchange(
        ...
        (uint dynamicFeeDst, bool dstVolatile) = _dynamicFeeRateForCurrency(
            destinationSettings
        );
        (uint dynamicFeeSrc, bool srcVolatile) = _dynamicFeeRateForCurrency(
            sourceSettings
        );
        ...
        dynamicFee = overMax
            ? sourceSettings.exchangeMaxDynamicFee
            : dynamicFee;
    ...
    /// @return dynamicFee The dynamic fee and if it exceeds max dynamic fee set in config
    ...
    function _dynamicFeeCalculation(
        ...
        for (uint i = prices.length - 1; i > 0; i--) {
            ...
            uint deviation = _thresholdedAbsDeviationRatio(
                prices[i - 1],
                prices[i],
                threshold
            );
    ...
    function _thresholdedAbsDeviationRatio(
        ...
        uint absDelta = price > previousPrice
            ? price - previousPrice
            : previousPrice - price;
