...
contract RewardEscrow is Ownable, IRewardEscrow {
    ...
    // An account's total escrowed SMX balance to save recomputing this for fee extraction purposes
    mapping(address => uint256) public override totalEscrowedAccountBalance;

    // An account's total vested reward SMX
    mapping(address => uint256) public override totalVestedAccountBalance;

    // The total remaining escrowed balance, for verifying the actual SMX balance of this contract against
    ...
    constructor(address _owner, address _smx) Ownable(_owner) {
        nextEntryId = 1;

        // set the SMX contract address as we need to transfer SMX when the user vests
        smx = ISMX(_smx);
    }
    ...
    /**
     * @notice helper function to return smx address
     */
    function getSMXAddress() external view override returns (address) {
        return address(smx);
    }
    ...
    ) external view override returns (uint total, uint totalFee) {
    ...
    function getVestingEntryClaimable(
        address account,
        uint256 entryID
    ) external view override returns (uint quantity, uint fee) {
    ...
    function _claimableAmount(
        VestingEntries.VestingEntry memory _entry
    ...
    function vest(uint256[] calldata entryIDs) external override {
        ...
        if (total != 0) {
            // Withdraw staked escrowed smx if needed for reward
            if (_isEscrowStaked(msg.sender)) {
                uint totalWithFee = total + totalFee;
                uint unstakedEscrow = totalEscrowedAccountBalance[msg.sender] -
            ...
            if (totalFee != 0) {
                ...
                require(
                    ISMX(address(smx)).transfer(treasuryDAO, totalFee),
    ...
    /**
     * @notice Create an escrow entry to lock SMX for a given duration in seconds
    ...
    function createEscrowEntry(
        ...
        /* Transfer SMX from msg.sender */
        require(
            smx.transferFrom(msg.sender, address(this), deposit),
     ...
     * @dev A call to this should accompany a previous successful call to smx.transfer(rewardEscrow, amount),
     * to ensure that when the funds are withdrawn, there is enough balance.
     * @param account The account to append a new vesting entry to.
     * @param quantity The quantity of SMX that will be escrowed.
     * @param duration The duration that SMX will be emitted.
     */
    ...
    /**
     * @notice Unstakes escrowed SMX.
     * @dev No tokens are transfered during this process, but the StakingRewards escrowed balance is updated.
     * @param _amount The amount of escrowed SMX to be unstaked.
     */
    ...
    function _transferVestedTokens(address _account, uint256 _amount) internal {
        ...
        smx.transfer(_account, _amount);
        emit Vested(_account, _amount);
    }

    function _reduceAccountEscrowBalances(
        address _account,
        uint256 _amount
    ) internal {
    ...
    function _appendVestingEntry(
        ...
        require(
            totalEscrowedBalance <= smx.balanceOf(address(this)),
