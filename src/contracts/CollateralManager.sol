...
import "../libraries/Bytes32SetLib.sol";
import "../libraries/SafeDecimalMath.sol";

contract CollateralManager is Ownable, Pausable, MixinResolver {
    /* ========== LIBRARIES ========== */
    ...
    constructor(
        ...
        uint _baseShortRate
    ) Ownable(msg.sender) Pausable() MixinResolver(_resolver) {
        state = CollateralManagerState(_state);
        ...
        returns (bytes32[] memory addresses)
    {
        ...
        bytes32[] memory synthAddresses = combineArrays(
            shortAddresses,
            _synths.elements
        );
        ...
        returns (uint cfusdValue, bool anyRateIsInvalid)
    {
        ...
        if (currencyKeys.length > 0) {
            (uint[] memory rates, bool invalid) = _exchangeRates()
                .ratesAndInvalidForCurrencies(currencyKeys);
            for (uint i = 0; i < rates.length; i++) {
                uint longAmount = state.long(currencyKeys[i]).multiplyDecimal(
                    rates[i]
                );
        ...
        returns (uint borrowRate, bool anyRateIsInvalid)
    {
        ...
        uint scaledUtilisation = utilisation.multiplyDecimal(
    ...
    function getShortRate(
        bytes32 synthKey
    ) public view returns (uint shortRate, bool rateIsInvalid) {
        rateIsInvalid = _exchangeRates().rateIsInvalid(synthKey);

        // Get the long and short supply.
        uint longSupply = IERC20(
            address(_synth(shortableSynthsByKey[synthKey]))
        ).totalSupply();
        ...
        uint proportionalSkew = skew
            .divideDecimal(longSupply.add(shortSupply))
            .divideDecimal(SECONDS_IN_A_YEAR);
    ...
    function getRatesAndTime(
        ...
        public
        view
        returns (uint entryRate, uint lastRate, uint lastUpdated, uint newIndex)
    {
        (entryRate, lastRate, lastUpdated, newIndex) = state.getRatesAndTime(
            index
        );
    }
    ...
    function getShortRatesAndTime(
        ...
        returns (uint entryRate, uint lastRate, uint lastUpdated, uint newIndex)
    {
        (entryRate, lastRate, lastUpdated, newIndex) = state
            .getShortRatesAndTime(currency, index);
    }

    function exceedsDebtLimit(
        uint amount,
        bytes32 currency
    ) external view returns (bool canIssue, bool anyRateIsInvalid) {
    ...
    function addSynths(
        ...
        require(
            synthNamesInResolver.length == synthKeys.length,
    ...
    function addShortableSynths(
        ...
        require(
            requiredSynthNamesInResolver.length == synthKeys.length,
            "Input array length mismatch"
        );
    ...
    function areShortableSynthsSet(
        ...
        if (
            _shortableSynths.elements.length !=
            requiredSynthNamesInResolver.length
        ) {
            return false;
        }
    ...
    function accrueInterest(
        ...
        (uint rate, bool invalid) = isShort
            ? getShortRate(currency)
            : getBorrowRate();
        ...
        isShort
            ? updateShortRates(currency, latestCumulative)
            : updateBorrowRates(latestCumulative);
    }
