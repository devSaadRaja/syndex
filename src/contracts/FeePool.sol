    ...
    IFeePool
{
    ...
    bytes32 private constant CONTRACT_FUTURES_MARKET_MANAGER =
        ...
        returns (IFuturesMarketManager)
    {
        return
            IFuturesMarketManager(
                requireAndGetAddress(CONTRACT_FUTURES_MARKET_MANAGER)
            );
    }
    ...
    function allNetworksDebtSharesSupply()
        public
        view
        returns (uint256 sharesSupply, uint256 updatedAt)
    {
        (
            ,
        ...
        updatedAt = issuedSynthsUpdatedAt < ratioUpdatedAt
    ...
    function _recentFeePeriodsStorage(
        uint index
    ) internal view returns (FeePeriod storage) {
        return
    ...
    function closeCurrentFeePeriod() external issuanceActive {
        ...
        require(
            _recentFeePeriodsStorage(0).startTime <=
                (block.timestamp - getFeePeriodDuration()),
    ...
    function _closeSecondary(
        ...
        if (_recentFeePeriodsStorage(0).feesToDistribute > 0) {
            ...
            _recentFeePeriodsStorage(0).feesClaimed = _recentFeePeriodsStorage(
                0
            ).feesToDistribute;
        }
    ...
    function _recordRewardPayment(uint sfcxAmount) internal returns (uint) {
        ...
        for (uint i = FEE_PERIOD_LENGTH - 1; i < FEE_PERIOD_LENGTH; i--) {
            uint toDistribute = _recentFeePeriodsStorage(i)
                .rewardsToDistribute
                .sub(_recentFeePeriodsStorage(i).rewardsClaimed);

            if (toDistribute > 0) {
                // Take the smaller of the amount left to claim in the period and the amount we need to allocate
                uint amountInPeriod = toDistribute < remainingToAllocate
                    ? toDistribute
                    : remainingToAllocate;
    ...
    function _isFeesClaimableAndAnyRatesInvalid(
        ...
        uint ratio_threshold = targetRatio.multiplyDecimal(
    ...
    function isFeesClaimable(
        address account
    ) external view returns (bool feesClaimable) {
    ...
    function feesByPeriod(
        ...
        for (uint i = FEE_PERIOD_LENGTH - 1; i > 0; i--) {
            ...
            if (lastFeeWithdrawal < periodId) {
                userOwnershipPercentage = sds.sharePercentOnPeriod(
                    account,
                    uint(periodId)
                );
    ...
    function _feesAndRewardsFromPeriod(
        ...
        uint feesFromPeriod = fp.feesToDistribute.multiplyDecimal(
            ownershipPercentage
        );

        uint rewardsFromPeriod = fp.rewardsToDistribute.multiplyDecimal(
            ownershipPercentage
        );
    ...
    bytes32 private constant FEEPERIODCLOSED_SIG =
        keccak256("FeePeriodClosed(uint256)");
