...
contract Issuer is Ownable, MixinSystemSettings {
    ...
    bytes32 private constant CONTRACT_EXT_AGGREGATOR_DEBT_RATIO =
    ...
    function liquidatorRewards() internal view returns (ILiquidatorRewards) {
        return
            ILiquidatorRewards(
                requireAndGetAddress(CONTRACT_LIQUIDATOR_REWARDS)
            );
    }
        ...
        returns (uint256 debt, uint256 sharesSupply, bool isStale)
    {
        (
            ,
            int256 rawIssuedSynths,
            ,
            uint issuedSynthsUpdatedAt,
        ...
        sharesSupply = rawRatio == 0
            ? 0
            : debt.divideDecimalRoundPrecise(uint(rawRatio));

        uint stalePeriod = getRateStalePeriod();

        isStale =
            stalePeriod < block.timestamp &&
            (block.timestamp - stalePeriod > issuedSynthsUpdatedAt ||
                block.timestamp - stalePeriod > ratioUpdatedAt);
    }
    ...
    function _sharesForDebt(uint debtAmount) internal view returns (uint) {
        ...
        return
            rawRatio == 0 ? 0 : debtAmount.divideDecimalRoundPrecise(rawRatio);
    }
    ...
    function _availableCurrencyKeysWithOptionalSNX(
        ...
        bytes32[] memory currencyKeys = new bytes32[](
            availableSynths.length + (withSNX ? 1 : 0)
        );
    ...
    function _totalIssuedSynths(
        ...
        (uint debt, , bool cacheIsInvalid, bool cacheIsStale) = debtCache()
            .cacheInfo();
    ...
    function _collateralisationRatio(
        ...
        (
            ...
            ,
            bool anyRateIsInvalid
        ) = _debtBalanceOfAndTotalDebt(_debtShareBalanceOf(_issuer), SNX);
    ...
    function _collateral(address account) internal view returns (uint) {
        return
            _snxBalanceOf(account).add(_rewardEscrowBalanceOf(account)).add(
        ...
        returns (bool anyRateInvalid)
    {
        (, anyRateInvalid) = exchangeRates().ratesAndInvalidForCurrencies(
            _availableCurrencyKeysWithOptionalSNX(true)
        );
    }
    ...
    ) external view returns (uint totalIssued) {
        (totalIssued, ) = _totalIssuedSynths(
            currencyKey,
            excludeOtherCollateral
        );
    }
    ...
    function modifyDebtSharesForMigration(
        ...
        if (
            msg.sender ==
            resolver.getAddress(CONTRACT_DEBT_MIGRATOR_ON_ETHEREUM)
        ) {
            sds.burnShare(account, amount);
        } else if (
            msg.sender ==
            resolver.getAddress(CONTRACT_DEBT_MIGRATOR_ON_OPTIMISM)
        ) {
    ...
    function upgradeCollateralShort(
        ...
        require(
            short == resolver.getAddress("CollateralShortLegacy"),
            "wrong address"
        );
    ...
    {
        require(
            liquidator().isLiquidationOpen(account, isSelfLiquidation),
        ...
        (
            ...
            escrowToLiquidate,
            initialDebtBalance
        ) = _liquidationAmounts(account, isSelfLiquidation);
        ...
        )
    {
        ...
        if (isSelfLiquidation) {
            ...
            totalRedeemed = _getMinValue(
                _usdToSnx(debtToRemove, snxRate).multiplyDecimal(
                    SafeDecimalMath.unit().add(penalty)
                ),
                _snxBalanceOf(account)
            );
            debtToRemove = _getMinValue(
                _snxToUSD(totalRedeemed, snxRate).divideDecimal(
                    SafeDecimalMath.unit().add(penalty)
                ),
                debtToRemove
            );
            ...
            uint collateralForAccountUSD = _snxToUSD(
            ...
            uint redeemTarget = _usdToSnx(debtToRemove, snxRate)
    ...
    function setCurrentPeriodId(uint128 periodId) external {
        require(
            msg.sender == requireAndGetAddress(CONTRACT_FEEPOOL),
            "Must be fee pool"
        );
    ...
    function _removeFromDebtRegister(
        ...
        if (debtToRemove == existingDebt) {
            ...
            sds.burnShare(
                from,
                sharesToRemove < currentDebtShare
                    ? sharesToRemove
                    : currentDebtShare
            );
        }
    ...
    function _verifyCircuitBreakers() internal returns (bool) {
        address debtRatioAggregator = requireAndGetAddress(
            CONTRACT_EXT_AGGREGATOR_DEBT_RATIO
        );
    ...
    modifier onlyTrustedMigrators() {
        address migratorL1 = resolver.getAddress(
            CONTRACT_DEBT_MIGRATOR_ON_ETHEREUM
        );
        address migratorL2 = resolver.getAddress(
            CONTRACT_DEBT_MIGRATOR_ON_OPTIMISM
        );
        require(
            msg.sender == migratorL1 || msg.sender == migratorL2,
            "only trusted migrators"
        );
        require(
            migratorL1 == address(0) || migratorL2 == address(0),
