...
contract DebtCache is BaseDebtCache {
    ...
    function purgeCachedSynthDebt(
        bytes32 currencyKey
    ) external override onlyOwner {
        require(
            issuer().synths(currencyKey) == ISynth(address(0)),
            "Synth exists"
        );
    ...
    ) external override requireSystemActiveIfNotOwner {
        (uint[] memory rates, bool anyRateInvalid) = exchangeRates()
    ...
    ) external override onlyIssuerOrExchanger {
        _updateCachedSynthDebtsWithRates(currencyKeys, currencyRates, false);
    }

    function updateDebtCacheValidity(
        bool currentlyInvalid
    ) external override onlyIssuer {
    ...
    function recordExcludedDebtChange(
        bytes32 currencyKey,
        int256 delta
    ) external override onlyDebtIssuer {
    ...
    function _updateCachedSynthDebtsWithRates(
        ...
        uint cachedSum;
        uint currentSum;
        uint[] memory currentValues = _issuedSynthValues(
            currencyKeys,
            currentRates
        );
