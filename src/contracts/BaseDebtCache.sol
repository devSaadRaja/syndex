...
contract BaseDebtCache is Ownable, MixinSystemSettings, IDebtCache {
    ...
    {
        (uint[] memory rates, bool isInvalid) = exchangeRates()
    ...
    function excludedIssuedDebts(
        bytes32[] calldata currencyKeys
    ) external view returns (uint[] memory excludedDebts) {
    ...
    function _totalNonSfcxBackedDebt(
        ...
        (uint longValue, bool anyTotalLongRateIsInvalid) = collateralManager()
            .totalLong();
        (uint shortValue, bool anyTotalShortRateIsInvalid) = collateralManager()
            .totalShort();
        isInvalid =
            ratesAreInvalid ||
    ...
    function _currentDebt()
        internal
        view
        returns (uint debt, bool anyRateIsInvalid)
    {
        bytes32[] memory currencyKeys = issuer().availableCurrencyKeys();
        (uint[] memory rates, bool isInvalid) = exchangeRates()
            .ratesAndInvalidForCurrencies(currencyKeys);
    ...
    function cacheInfo()
        external
        view
        returns (uint debt, uint timestamp, bool isInvalid, bool isStale)
    {
    ...
    function updateCachedSynthDebts(
        bytes32[] calldata currencyKeys
    ) external virtual {}
    ...
    function recordExcludedDebtChange(
        bytes32 currencyKey,
        int256 delta
    ) external virtual {}
