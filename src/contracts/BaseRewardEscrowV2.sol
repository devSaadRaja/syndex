...
contract BaseRewardEscrowV2 is
    Ownable,
    IRewardEscrowV2,
    LimitedSetup(8 weeks),
    MixinResolver
{
    ...
    function state() internal view returns (IRewardEscrowV2Storage) {
        return
            IRewardEscrowV2Storage(
                requireAndGetAddress(CONTRACT_REWARDESCROWV2STORAGE)
            );
    }
    ...
    function vestingSchedules(
        address account,
        uint256 entryId
    ) public view returns (VestingEntries.VestingEntry memory) {
     ...
     * @return endTime escrowAmount The vesting entry object and rate per second emission.
     */
    function getVestingEntry(
        address account,
        uint256 entryID
    ) external view returns (uint64 endTime, uint256 escrowAmount) {
    ...
    function vest(uint256[] calldata entryIDs) external {
        ...
        if (total != 0) {
            ...
            // update total vested
            state().updateVestedAccountBalance(
                account,
                SafeCast.toInt256(total)
            );
    ...
    function revokeFrom(
        ...
        (uint total, uint endIndex, uint lastEntryTime) = state()
            .setZeroAmountUntilTarget(account, startIndex, targetAmount);
        ...
        if (total > targetAmount) {
            ...
            uint entryID = state().addVestingEntry(
                ...
                VestingEntries.VestingEntry({
                    endTime: uint64(lastEntryTime),
                    escrowAmount: refund
                })
            );
            // emit event
            uint duration = lastEntryTime > block.timestamp
                ? lastEntryTime.sub(block.timestamp)
                : 0;
    ...
    function setAccountMergingDuration(uint256 duration) external onlyOwner {
        require(
            duration <= maxAccountMergingDuration,
            "exceeds max merging duration"
        );
    ...
    function mergeAccount(address from, uint256[] calldata entryIDs) external {
        ...
        require(
            nominatedReceiver[from] == msg.sender,
        ...
        for (uint i = 0; i < entryIDs.length; i++) {
            ...
            if (entry.escrowAmount != 0) {
                ...
                totalEscrowAmountMerged = totalEscrowAmountMerged.add(
                    entry.escrowAmount
                );
            }
        }
        ...
        state().updateEscrowAccountBalance(
            from,
            -SafeCast.toInt256(totalEscrowAmountMerged)
        );
    ...
    function burnForMigration(
        address,
        uint[] calldata
    ) external virtual returns (uint256, VestingEntries.VestingEntry[] memory) {
        _notImplemented();
    }
