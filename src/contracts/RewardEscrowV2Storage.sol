...
contract RewardEscrowV2Storage is IRewardEscrowV2Storage, State {
    ...
    mapping(address => mapping(uint => StorageEntry))
        internal _vestingSchedules;
    ...
    // account's total escrow SFCX balance (still to vest)
    ...
    function setFallbackRewardEscrow(
        address _fallbackRewardEscrow
    ) external onlyOwner {
        require(address(fallbackRewardEscrow) == address(0), "already set");
        require(
            address(_fallbackRewardEscrow) != address(0),
            "cannot be zero address"
        );

        fallbackRewardEscrow = IRewardEscrowV2Frozen(_fallbackRewardEscrow);
        nextEntryId = IRewardEscrowV2Frozen(_fallbackRewardEscrow).nextEntryId();
        ...
        returns (VestingEntries.VestingEntry memory entry)
    {
        ...
        entry = VestingEntries.VestingEntry({
            endTime: stored.endTime,
            escrowAmount: stored.escrowAmount
        });
    ...
    function accountVestingEntryIDs(
        address account,
        uint index
    ) public view withFallback returns (uint) {
    ...
    function numVestingEntries(
        ...
        return
            _fallbackNumVestingEntries(account) +
            _accountVestingEntryIds[account].length;
    }

    /* ========== INTERNAL VIEWS ========== */

    function _fallbackNumVestingEntries(
        address account
    ...
    function setZeroAmount(
        address account,
        uint entryId
    ) public withFallback onlyAssociatedContract {
        // load storage entry
    ...
    function setZeroAmountUntilTarget(
        ...
        returns (uint total, uint endIndex, uint lastEntryTime)
    {
    ...
    function updateEscrowAccountBalance(
        address account,
        int delta
    ) external withFallback onlyAssociatedContract {
        // add / subtract to previous balance
        int total = int(totalEscrowedAccountBalance(account)).add(delta);
        require(
            total >= 0,
            "updateEscrowAccountBalance: balance must be positive"
        );
        ...
        _totalEscrowedAccountBalance[account] = _writeWithZeroPlaceholder(
            uint(total)
        );
    ...
    function updateVestedAccountBalance(
        address account,
        int delta
    ) external withFallback onlyAssociatedContract {
        // add / subtract to previous balance
        ...
        require(
            total >= 0,
            "updateVestedAccountBalance: balance must be positive"
        );
        // zero value must never be written, because it is used to signal uninitialized
        //  writing an actual 0 will result in stale value being read from fallback
        // casting is safe because checked above
        _totalVestedAccountBalance[account] = _writeWithZeroPlaceholder(
            uint(total)
        );
    }
    ...
    function updateTotalEscrowedBalance(
        int delta
    ) public withFallback onlyAssociatedContract {
        int total = int(totalEscrowedBalance()).add(delta);
        require(
            total >= 0,
            "updateTotalEscrowedBalance: balance must be positive"
        );
    ...
    function _cacheFallbackIDCount(address account) internal {
        if (_fallbackCounts[account] == 0) {
            uint fallbackCount = fallbackRewardEscrow.numVestingEntries(
                account
            );
    ...
    function _readWithZeroPlaceholder(int v) internal pure returns (uint) {
        // 0 is uninitialized value, so a special value is used to store an actual 0 (that is initialized)
        return uint(v == ZERO_PLACEHOLDER ? int256(0) : v);
    }
    ...
    modifier withFallback() {
        require(
            address(fallbackRewardEscrow) != address(0),
            "fallback not set"
        );
        _;
    }
