 ...
 */
contract CircuitBreaker is Ownable, MixinSystemSettings {
    ...
    // TODO: after upgrading solidity, switch to just public lastValue instead of maintaining this internal one
    ...
    function probeCircuitBreaker(
        address oracleAddress,
        uint value
    ) external onlyProbers returns (bool) {
    ...
    function resetLastValue(
        ...
        for (uint i = 0; i < oracleAddresses.length; i++) {
            ...
            emit LastValueOverridden(
                oracleAddresses[i],
                _lastValue[oracleAddresses[i]],
                values[i]
            );
    ...
    function _isDeviationAboveThreshold(
        uint base,
        uint comparison
