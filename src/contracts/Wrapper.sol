...
contract Wrapper is Ownable, Pausable, MixinSystemSettings, IWrapper {
    ...
    function mint(uint amountIn) external whenNotPaused issuanceActive {
        require(
            amountIn <= token.allowance(msg.sender, address(this)),
        ...
        uint actualAmountIn = currentCapacity < amountIn
            ? currentCapacity
            : amountIn;

        (uint feeAmountTarget, bool negative) = calculateMintFee(
            actualAmountIn
        );
        uint mintAmount = negative
            ? actualAmountIn.add(feeAmountTarget)
            : actualAmountIn.sub(feeAmountTarget);
    ...
    function burn(uint amountIn) external whenNotPaused issuanceActive {
        require(
            amountIn <= IERC20(address(synth())).balanceOf(msg.sender),
            "Balance is too low"
        );
        require(
            !exchangeRates().rateIsInvalid(currencyKey),
        ...
        uint amountOut;
        if (negative) {
            burnAmount = targetSynthIssued < amountIn
                ? targetSynthIssued.sub(burnFee)
                : amountIn;
            ...
            burnAmount = targetSynthIssued.add(burnFee) < amountIn
                ? targetSynthIssued.add(burnFee)
                : amountIn;
    ...
    function _mint(uint amount) internal {
        ...
        uint excessAmount = reserves > targetSynthIssued.add(amount)
            ? reserves.sub(targetSynthIssued.add(amount))
            : 0;
    ...
    function _burn(uint amount) internal {
        ...
        uint excessAmount = reserves.add(amount) > targetSynthIssued
            ? reserves.add(amount).sub(targetSynthIssued)
            : 0;
    ...
    function _setTargetSynthIssued(uint _targetSynthIssued) internal {
        debtCache().recordExcludedDebtChange(
            currencyKey,
            int256(_targetSynthIssued) - int256(targetSynthIssued)
        );
