...
contract Collateral is ICollateralLoan, Ownable, MixinSystemSettings {
    /* ========== LIBRARIES ========== */
    ...
    function liquidationAmount(uint id) public view returns (uint liqAmount) {
        Loan memory loan = loans[id];
        return
            _collateralUtil().liquidationAmount(loan, minCratio, collateralKey);
    }
    ...
    function _checkSynthBalance(
        ...
        require(
            IERC20(address(_synth(synthsByKey[key]))).balanceOf(payer) >=
                amount,
            "Not enough balance"
        );
    }
    ...
    function _checkLoanAvailable(Loan memory loan) internal view {
        ...
        require(
            loan.lastInteraction.add(getInteractionDelay(address(this))) <=
                block.timestamp,
            "Recently interacted"
        );
    }
    ...
    function addSynths(
        ...
        require(
            _synthNamesInResolver.length == _synthKeys.length,
    ...
    function _open(
        ...
        require(
            amount <= maxLoan(collateral, currency),
        ...
        if (short) {
            ...
            if (shortingRewards[currency] != address(0)) {
                IShortingRewards(shortingRewards[currency]).enrol(
                    msg.sender,
                    amount
                );
            }
    ...
    function _close(
        ...
        rateIsValid
        issuanceIsActive
        returns (uint amount, uint collateral)
    {
    ...
    function _closeLoan(
        ...
        if (loan.short) {
            ...
            if (shortingRewards[loan.currency] != address(0)) {
                IShortingRewards(shortingRewards[loan.currency]).withdraw(
                    borrower,
                    loan.amount
                );
            }
    ...
    function _liquidate(
        ...
        rateIsValid
        issuanceIsActive
        returns (uint collateralLiquidated)
    {
        ...
        require(
            _collateralUtil().getCollateralRatio(loan, collateralKey) <
                minCratio,
            "Cratio above liq ratio"
        );

        // 3. Determine how much needs to be liquidated to fix their c ratio.
        uint liqAmount = _collateralUtil().liquidationAmount(
            loan,
            minCratio,
            collateralKey
        );
        ...
        if (amountToLiquidate >= amountOwing) {
            (, collateralLiquidated) = _closeByLiquidation(
                borrower,
                msg.sender,
                loan
            );
        ...
        collateralLiquidated = _collateralUtil().collateralRedeemed(
            loan.currency,
            amountToLiquidate,
            collateralKey
        );
    ...
    function _draw(
        ...
        if (loan.short) {
            ...
            if (shortingRewards[loan.currency] != address(0)) {
                IShortingRewards(shortingRewards[loan.currency]).enrol(
                    msg.sender,
                    amount
                );
            }
            ...
            _synth(synthsByKey[loan.currency]).issue(
                msg.sender,
                amountMinusFee
            );
        }
    ...
    function _accrueInterest(Loan storage loan) internal {
        (uint differential, uint newIndex) = manager.accrueInterest(
            loan.interestIndex,
            loan.currency,
            loan.short
        );
        ...
        uint interest = loan.interestIndex == 0
            ? 0
            : loan.amount.multiplyDecimal(differential);
    ...
    function _processPayment(Loan storage loan, uint payment) internal {
        ...
        if (loan.accruedInterest > 0) {
            uint interestPaid = payment > loan.accruedInterest
    ...
    function _getLoanAndAccrueInterest(
        uint id,
        address owner
