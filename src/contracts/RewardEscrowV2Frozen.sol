...
contract RewardEscrowV2Frozen is BaseRewardEscrowV2Frozen {
    ...
    constructor(
        address _owner,
        address _resolver
    ) BaseRewardEscrowV2Frozen(_owner, _resolver) {}
    ...
    function migrateVestingSchedule(
        address addressToMigrate
    ) external override systemActive {
        ...
        require(
            totalBalancePendingMigration[addressToMigrate] > 0,
            "No escrow migration pending"
        );
        require(
            totalEscrowedAccountBalance[addressToMigrate] > 0,
        ...
        if (
            ...
            uint numEntries = oldRewardEscrow().numVestingEntries(
                addressToMigrate
            );
            ...
            for (uint i = 1; i <= numEntries; i++) {
                uint[2] memory vestingSchedule = oldRewardEscrow()
                    .getVestingScheduleEntry(addressToMigrate, numEntries - i);
                ...
                totalBalancePendingMigration[
                    addressToMigrate
                ] = totalBalancePendingMigration[addressToMigrate].sub(amount);
            }
        }
    }
    ...
    function importVestingSchedule(
        ...
        require(
            accounts.length == escrowAmounts.length,
            "Account and escrowAmounts Length mismatch"
        );
        ...
        for (uint i = 0; i < accounts.length; i++) {
            ...
            totalBalancePendingMigration[
                addressToMigrate
            ] = totalBalancePendingMigration[addressToMigrate].sub(
                escrowAmount
            );

            emit ImportedVestingSchedule(
                addressToMigrate,
                block.timestamp,
                escrowAmount
            );
    ...
    function migrateAccountEscrowBalances(
        ...
        require(
            accounts.length == escrowBalances.length,
            "Number of accounts and balances don't match"
        );
        require(
            accounts.length == vestedBalances.length,
            "Number of accounts and vestedBalances don't match"
        );
        ...
        for (uint i = 0; i < accounts.length; i++) {
            ...
            require(
                totalBalancePendingMigration[account] == 0,
    ...
    function _importVestingEntry(
        ...
        uint entryID = BaseRewardEscrowV2Frozen._addVestingEntry(
            account,
            entry
        );
    ...
    /* ========== EVENTS ========== */
    event MigratedAccountEscrow(
        address indexed account,
        uint escrowedAmount,
        uint vestedAmount,
        uint time
    );
    event ImportedVestingSchedule(
        address indexed account,
        uint time,
        uint escrowAmount
    );
